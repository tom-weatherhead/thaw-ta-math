...
function diffAdjacentElements(array: number[]): number[] {
	// Difference between adjacent elements; i.e.:
	...
	for (const elt of array.slice(1)) {
		result.push(elt - prev);
		prev = elt;
	}
...
export function adx(
	...
	const adx = createNaNArray(window).concat(
		ema(dx.slice(window), 2 * window - 1)
	);
...
// the change in trading range between periods. This means that if there is
// any type of gap in price, it won’t be picked up on and therefore Chaikin
// Money Flow and price will become out of sync.
...
// Accumulation/Distribution (ADL) and The Chaikin Oscillator (cho).
...
// (Not Bill Williams' Market Facilitation Index)
...
function mapRatioToCenturyRange(a: number, b: number) {
...
export function mfi(
	...
	return pointwise(
		mapRatioToCenturyRange, // (a: number, b: number) => 100 - 100 / (1 + a / b),
		xmf(isNonNegative), // pmf, // -> Positive Money Flow
		xmf(isNegative) // nmf // -> Negative Money Flow
	);
}
